...
exports.generateSessionToken = onCall({ region: 'europe-west1' }, async (request) => {
  if (!request.auth) {
...
exports.submitScore = onCall({ region: 'europe-west1' }, async (request) => {
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'Authentification requise.');
  }
...
exports.updateUsername = onCall({ region: 'europe-west1' }, async (request) => {
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'Authentification requise.');
  }
